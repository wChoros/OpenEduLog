...
attendanceRouter.get('/student/:studentId', authorize('read', 'Attendance'), async (req, res) => {
...
// Justify absences
attendanceRouter.post('/justify', authorize('update', 'Attendance'), async (req, res) => {
   ...
   try {
      ...
      const records = await prisma.attendance.findMany({
         where: {
            id: { in: attendanceIds.map((id: string | number) => parseInt(id as string, 10)) },
         },
      })
